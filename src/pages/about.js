...
const AboutPage = ({ data }) => {
  ...
  return (
    <>
      <Layout>
      <SEO title="About" description="something personal" image={user.avatarUrl}/>
        <div className="py-20 grid content-center gap-10 h-screen">
          <div className="justify-self-center text-center">
            <a href={user.url}>
              <img src={user.avatarUrl} className="h-20 w-20 rounded-full mx-auto"></img>
            </a>
            <p className="text-lg p-3 font-bold font-mono">@{user.login}</p>
            <div className="text-sm p-3">
              <span>{user.status.emojiHTML.match(/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g)}</span>
